function [label_idx,scores] = train_grain(img_name,slic,T)
    %% 谷粒参数设置
    m1=[77.1993;46.5755]; %前景均值
    C1=[0.0032 -0.0058;-0.0058 0.0228];%前景协方差矩阵
    m2=[2.3856;2.4703]; %背景均值
    C2=[0.1071,-0.1358;-0.1358,0.2346]; %背景协方差矩阵
    ...
    for i=1:size1(1)
        for j=1:size1(2)
                ...
                x1(2)=lab(i,j,3)-m1(2);
                temp=-0.5*x1'*C1*x1;
                p1(i,j)=exp(temp);%计算各点属于谷粒的概率

                x2(1)=lab(i,j,2)-m2(1);
                x2(2)=lab(i,j,3)-m2(2);
                temp=-0.5*x2'*C2*x2;
                p2(i,j)=exp(temp);%计算各点属于背景的概率
