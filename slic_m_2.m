function [slic_result2,num_superpixel] = slic_m_2(I,num_superpixel,n_iter,minsize_superpixel,w1,w2,I_recognize)
%SLIC_M 此处显示有关此函数的摘要
%% 读取图像
...
seed_step=ceil(sqrt(image_pixels_number/num_superpixel));
seed_step_half=ceil(seed_step/2);
%定义种子点变量，存放五个特征
...
each_num_superpixel=zeros(max_label,2);
for i=1:max_label
    t_label=find(slic_result2==i);
    each_num_superpixel(i,1)=i;
    each_num_superpixel(i,2)=length(t_label);
end

small_count=each_num_superpixel(each_num_superpixel(:,2)<minsize_superpixel,:);
while ~isempty(small_count)
    small_label=small_count(1,1);
    if small_count(1,2)>minsize_superpixel
        small_count(1,:)=[];
        continue;
    end
    neighbor=[];
    [ga_x,ga_y]=find(slic_result2==small_label);
    for k=1:length(ga_x)
        for ii=-1:1
            for jj=-1:1
                if (ii==0 && jj==0) || (ga_x(k)+ii<1) || (ga_x(k)+ii>size_I(1)) || (ga_y(k)+jj<1) || (ga_y(k)+jj>size_I(2))
                    continue;
                end
                neighbor=[neighbor ;[ga_x(k)+ii ga_y(k)+jj]];
            end
        end
    end
    neighbor=unique(neighbor,'rows');
    [~,Locb]=ismember([ga_x,ga_y],neighbor,'rows');
    if Locb~=0
        neighbor([Locb],:)=[];
    end

    neighbor=sub2ind([size_I(1),size_I(2)],neighbor(:,1),neighbor(:,2));
    label_neighbor=slic_result2(neighbor);
    label_neighbor=unique(label_neighbor,'rows');
    label_neighbor(label_neighbor==0)=[];
    if isempty(label_neighbor)
        small_count(1,:)=[];
        continue;
    end

    [Lic2,~]=ismember(small_count(:,1),label_neighbor,'rows');

    if ~all(Lic2(:)==0)
        [t_1,~]=find(Lic2==1);
        for jj=1:length(t_1)
            loca=t_1(jj);
            slic_result2(slic_result2==small_count(Lic2(loca),1))=small_label;
        end
        small_count(1,2)=small_count(1,2)+sum(small_count(Lic2,2));
        small_count(Lic2,:)=[];
        continue;
    end
    label=mode(label_neighbor);
    slic_result2(slic_result2==small_label)=label;
    small_count(1,:)=[];
end
...
for i=1:max_label
    t_label=find(slic_result2==i);
    each_num_superpixel(i,1)=length(t_label);
end
...
%     end
%     [bian_x,~]=find(each_num_superpixel<minsize_superpixel);
% end
